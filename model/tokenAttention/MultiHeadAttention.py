...
from model.tokenAttention import Attention
import config
...
class MultiHeadAttention(nn.Module):
	...
	def __init__(self,  d_model, n_head, attention_mask = None):
	...
	def forward(self, query, key, val, key_structure = None, val_structure = None, attention_mask = None):
		...
		if config.gpu == True:
			...
			query_head = Variable(torch.zeros((self.n_head, *query.shape[:-1], self.d_k), device = torch.device("cuda:0")))
			key_head = Variable(torch.zeros((self.n_head, *query.shape[:-1], self.d_k), device = torch.device("cuda:0")))
			val_head = Variable(torch.zeros((self.n_head, *query.shape[:-1], self.d_k), device = torch.device("cuda:0")))
