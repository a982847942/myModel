...
def loadTree(dataname):
    ...
    if dataname == 'SemEval':
        treePath = os.path.join(cwd,'data/SemEval/semevaltree.txt')
        print("reading SemEval tree")
        treeDic = {}
        for line in open(treePath):
            line = line.rstrip()
            eid, indexP, indexC,Vec = line.split('\t')[0], line.split('\t')[1], int(line.split('\t')[2]),line.split('\t')[3]
    ...
    if dataname == 'PHEME':
        ...
        for line in open(treePath):
            line = line.rstrip()
            #事件ID  父亲结点（当前事件图中） 自己的结点编号（当前事件图中） 词向量
            eid, indexP, indexC,Vec = line.split('\t')[0], line.split('\t')[1], int(line.split('\t')[2]),line.split('\t')[3]
            if not treeDic.__contains__(eid):
                treeDic[eid] = {}
            treeDic[eid][indexC] = {'parent': indexP, 'vec': Vec}
...
def loadData(dataname, treeDic,fold_x_train,fold_x_test,droprate):
    data_path=os.path.join(cwd, 'process/pheme/', dataname + 'Usergraph')
...
def loadUdData(dataname, treeDic,fold_x_train,fold_x_test,droprate):
    data_path=os.path.join(cwd, 'process/pheme/', dataname + 'graph')
    print("loading train set", )
